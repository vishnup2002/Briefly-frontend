  ...
  return (
    ...
    <div className="container">
      {meetings.map((meetings, index) => (
        <div key={index} className='d-flex flex-wrap justify-content-between ps-2 pe-2 mb-4 border-bottom'>
            <Link to = {`/meetings/${meetings.id}`} className="ps-5 fs-5  px-2 text-dark">{meetings.meetingName}</Link>
